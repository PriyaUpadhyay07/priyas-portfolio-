...
const Projects = () => {
  ...
  const projects = [
    ...
    },
    {
      ...
      description:
        "A stunning animated e-commerce website design showcasing modern fashion with smooth transitions and engaging visuals.",
      link: null,
      color: "bg-highlight-pink",
      image: fashionEcommerce1,
      isGallery: true,
      builtWith: ["v0 by Vercel", "Google AI Studio"],
      galleryItems: [
  ...
  const renderCardInner = (project: typeof projects[0], actionLabel: string) => (
    <>
      ...
      {project.builtWith && project.builtWith.length > 0 && <BuiltWith tools={project.builtWith} />}
          ...
          {projects.map((project, index) => {
            ...
            if (hasLink) {
              return (
                ...
                >
                  {renderCardInner(project, "View Project")}
                </a>
              );
            }
            ...
            if (isGallery) {
              return (
                <div
                  ...
                  className="group bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-8 shadow-lg border border-foreground/10 hover:shadow-2xl hover:-translate-y-3 transition-all duration-300 cursor-pointer"
                >
                  {renderCardInner(project, "View Designs")}
                </div>
              );
            }
              ...
              <div className="flex flex-col gap-6">
                {selectedGallery?.map((item, index) => (
                  <div key={index} className="w-full rounded-xl overflow-hidden shadow-lg">
                    {item.type === "video" ? (
                      <video src={item.src} controls autoPlay className="w-full h-auto" />
                    ) : (
                      <img src={item.src} alt={`Design ${index + 1}`} className="w-full h-auto object-contain" />
                    )}
                  </div>
                ))}
              </div>
